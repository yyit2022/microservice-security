...
导航到你从命令行应用程序从 Git 仓库克隆示例的目录，然后转到 chapter02/sample03 目录。从该目录中，执行以下 Maven 命令以构建示例：
...
然后从 chapter02/sample03/ 目录运行以下命令以启动安全的订单处理微服务。在这里，我们使用了一个名为 spring-boot 的 Maven 插件：
...
以下清单（chapter02/sample03/src/main/java/com/yyit/mss/ch02/sample03/configuration/SpringSecurityConfiguration.java 类文件）解释了资源服务器如何执行这些规则。
```java
@EnableWebSecurity
